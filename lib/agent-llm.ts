...
function buildSystemPrompt(state: ConversationState) {
  ...
  return [
    ...
    'Do NOT skip stages. Each stage must be completed before moving to the next.',
    ...
    '- Stage "intent": Occasion/vibe collected. Ask "What message should it say?". Stay at "intent" until moving to "text".',
    '- Stage "text": User provides text for design. MUST set updates.text. Move to "icon" ONLY after text is set.',
    '- Stage "icon": Text is set. Ask about icon. Move to "preview" when icon is chosen.',
    '- Stage "preview": Design complete. Allow cart actions.',
    'NEVER jump from "product" to "icon" - you MUST go through "intent" -> "text" first.',
    '',
