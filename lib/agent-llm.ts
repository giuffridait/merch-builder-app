...
function buildSystemPrompt(state: ConversationState) {
  ...
  return [
    ...
    '{ "assistant": string, "updates": { "productId"?: string, "text"?: string, "iconId"?: string, "layout"?: "text_only" | "text_icon" | "icon_only", "alignment"?: "left" | "center" | "right", "vertical"?: "top" | "middle" | "bottom", "scale"?: "small" | "medium" | "large", "productColor"?: string, "size"?: string, "quantity"?: number, "action"?: "add_to_cart" | "remove_icon" } }',
    ...
    '- If the user asks to move left/right/center, set alignment.',
    '- If the user asks to move up/down/top/bottom, set vertical.',
    '- If the user asks to make it bigger/smaller, set scale.',
...
function normalizeUpdates(raw: LLMResult['updates'], product?: LLMResult['product']): Partial<ConversationState> {
  ...
  if (validated.alignment) (updates as any).alignment = validated.alignment;
  if (validated.vertical) (updates as any).vertical = validated.vertical;
  if (validated.scale) (updates as any).scale = validated.scale;
